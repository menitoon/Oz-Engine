class Sprite:
    ...
    def __init__(
        ...
        # group is a string that be used to call a method on each sprite that has the same method.
    ...
    def update_position(self, new_position):
        ...
        if not self.canvas_owner.position_dict[old_position_tuple]:
    ...
    def update_camera_render_cache(self, camera):
        ...
        if camera.is_renderable(self.position):
            ...
            # check if empty
            if not camera.row_render_dict[row][x]:
    ...
    def kill(self):
        ...
        index_place = self.canvas_owner.sprite_group_dict[self.group].index(self)
        del (self.canvas_owner.sprite_group_dict[self.group])[index_place]
